...
def POA(SearchAgents, Max_iterations, lowerbound, upperbound, dimension, fitness, alpha=1, beta=1, rho=0.95, pheromone_init=0.1):
    ...
    pheromone = np.ones((SearchAgents, dimension)) * pheromone_init
    ...
    for t in range(1, Max_iterations + 1):
        ...
        for i in range(SearchAgents):
            ...
            # ----- ACO Phase (Ant Colony Optimization) -----
            # 1. Selection based on pheromone level
            prob = pheromone[i, :] ** alpha * (1 / (1 + fit[i])) ** beta  # Compute probability based on pheromone and fitness
            prob /= np.sum(prob)  # Normalize probabilities

            # 2. Explore the space based on pheromone probabilities
            chosen_idx = np.random.choice(dimension, size=1, p=prob)[0]
            X_new = X[i, :].copy()
            X_new[chosen_idx] = lowerbound[chosen_idx] + np.random.rand() * (upperbound[chosen_idx] - lowerbound[chosen_idx])
        ...
        # Update pheromones based on fitness (evaporation + deposition)
        pheromone = pheromone * (1 - rho)  # Evaporation
        for i in range(SearchAgents):
            pheromone[i, :] += 1 / (1 + fit[i])  # Deposition (inverse of fitness)
